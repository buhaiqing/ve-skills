...
description: >-
  GCL rubric instance for ve-polar-mysql-ops. Use to score Generator outputs
  ...
  Destructive: DeleteDBCluster; State-changing: ModifyDBNodeSpec, ScaleStorage, Failover, RestartDBInstance, ModifyDBInstanceParameter, ModifyDBInstanceIPList, ModifyDBEndpoint; Mutating: CreateDBCluster, CreateDBAccount, CreateBackup, RestoreToNewInstance, CreateReadOnlyNode; Read-only: DescribeDBClusters, DescribeDBClusterDetail, DescribeDBNodes, DescribeDBInstanceParameters, DescribeDBAccounts
license: MIT
metadata:
  author: volcengine
  version: "1.0.0"
...
| Tier | Operations in `ve-polar-mysql-ops` | `max_iter` | Safety floor |
|---|---|---|---|
...
- [ ] Full `ve polardb_mysql` command line (with resolved values, NOT templates)
...
- [ ] **Dual-path**: BOTH `ve polardb_mysql` CLI step AND Go SDK fallback step exist for every operation.
...
| Version | Date | Change |
|---|---|---|
| 1.0.0 | 2026-06-04 | Initial GCL rubric for ve-polar-mysql-ops (5 dimensions, 4-tier operation classification, product-specific safety + correctness + spec-compliance checks) |
