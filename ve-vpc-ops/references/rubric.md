...
description: >-
  GCL rubric instance for ve-vpc-ops. Use to score Generator outputs
  ...
  Destructive: DeleteVpc, DeleteSubnet; State-changing: CreateRouteEntry, DeleteRouteEntry, ModifyVpcAttribute; Mutating: CreateVpc, CreateSubnet, CreateRouteTable; Read-only: DescribeVpcs, DescribeSubnets, DescribeRouteTables
license: MIT
metadata:
  author: volcengine
  version: "1.0.0"
...
| Tier | Operations in `ve-vpc-ops` | `max_iter` | Safety floor |
|---|---|---|---|
| **Destructive** | `DeleteVpc, DeleteSubnet` | 3 | 1.0 (mandatory) |
| **State-changing** | `CreateRouteEntry, DeleteRouteEntry, ModifyVpcAttribute` | 3 | 1.0 (mandatory) |
...
- [ ] CreateVpc: `$.Result.VpcId` present; `DescribeVpcs` confirms status `Available`.
- [ ] DeleteVpc: `DescribeVpcs` returns not found within poll budget.
- [ ] CreateSubnet: `$.Result.SubnetId` present; `DescribeSubnets` confirms.
...
- [ ] DeleteVpc MUST verify empty (no subnets/route tables).
- [ ] DeleteSubnet MUST verify no resources.
...
- [ ] Full `ve vpc` command line (with resolved values, NOT templates)
...
- [ ] **Dual-path**: BOTH `ve vpc` CLI step AND Go SDK fallback step exist for every operation.
...
| Version | Date | Change |
|---|---|---|
| 1.0.0 | 2026-06-04 | Initial GCL rubric for ve-vpc-ops (5 dimensions, 4-tier operation classification, product-specific safety + correctness + spec-compliance checks) |
