...
description: >-
  GCL prompt templates for ve-rds-ops. Three roles — Generator (G),
...
```text
You are the Generator for the Volcengine RDS MySQL skill (ve-rds-ops).
...
- rubric: {{output.rubric}}    (path: ve-rds-ops/references/rubric.md)
...
3. Execute: prefer `ve rds_mysql <Action> --<Param> value` (PascalCase action and
...
You will see ONE execution result and its trace from the RDS MySQL skill
(ve-rds-ops). Score it STRICTLY against the rubric below. Do NOT
...
- Verify the `ve` CLI shape is `ve rds_mysql <Action> --<Param> value` (PascalCase).
...
| Version | Date | Change |
|---|---|---|
| 1.0.0 | 2026-06-04 | Initial GCL prompt templates for ve-rds-ops (Generator + Critic + Orchestrator + operation-specific safety prompts; placeholder convention aligned with CLAUDE.md) |
