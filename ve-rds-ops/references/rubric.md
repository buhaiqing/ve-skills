...
description: >-
  GCL rubric instance for ve-rds-ops. Use to score Generator outputs
  ...
  Destructive: DeleteDBInstance, DeleteDBAccount; State-changing: ModifyDBNodeSpec, ModifyDBInstanceParameter; Mutating: CreateDBInstance, CreateDBAccount, CreateBackup; Read-only: DescribeDBInstances, DescribeDBInstanceDetail, DescribeDBInstanceParameters, DescribeBackups
license: MIT
metadata:
  author: volcengine
  version: "1.0.0"
...
| Tier | Operations in `ve-rds-ops` | `max_iter` | Safety floor |
|---|---|---|---|
| **Destructive** | `DeleteDBInstance, DeleteDBAccount` | 2 | 1.0 (mandatory) |
| **State-changing** | `ModifyDBNodeSpec, ModifyDBInstanceParameter` | 2 | 1.0 (mandatory) |
...
- [ ] CreateDBInstance: `$.Result.InstanceId` present; status `RUNNING` within poll budget.
- [ ] DeleteDBInstance: `DescribeDBInstances` returns not found within poll budget.
- [ ] ModifyDBNodeSpec: new spec reflected; status returns `RUNNING`.
...
- [ ] DeleteDBInstance check deletion protection; irreversible.
- [ ] ModifyDBNodeSpec downtime 60-900s.
- [ ] Password masked. VOLCENGINE_SECRET_KEY never in trace.
...
- [ ] Full `ve rds_mysql` command line (with resolved values, NOT templates)
...
- [ ] **Dual-path**: BOTH `ve rds_mysql` CLI step AND Go SDK fallback step exist for every operation.
...
| Version | Date | Change |
|---|---|---|
| 1.0.0 | 2026-06-04 | Initial GCL rubric for ve-rds-ops (5 dimensions, 4-tier operation classification, product-specific safety + correctness + spec-compliance checks) |
