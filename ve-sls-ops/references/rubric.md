...
description: >-
  GCL rubric instance for ve-sls-ops. Use to score Generator outputs
  ...
  Destructive: DeleteProject, DeleteTopic, DeleteShipper; State-changing: CreateProject, CreateTopic, CreateShipper, ModifyProject, ModifyTopic, ModifyShipper, CreateIndex, ModifyIndex; Read-only: DescribeProjects, DescribeTopics, DescribeShippers, SearchLog, DescribeLogHistogram
license: MIT
metadata:
  author: volcengine
  version: "1.0.0"
...
| Tier | Operations in `ve-sls-ops` | `max_iter` | Safety floor |
|---|---|---|---|
| **Destructive** | `DeleteProject, DeleteTopic, DeleteShipper` | 5 | 1.0 (mandatory) |
| **State-changing** | `CreateProject, CreateTopic, CreateShipper, ModifyProject, ModifyTopic, ModifyShipper, CreateIndex, ModifyIndex` | 5 | 1.0 (mandatory) |
...
- [ ] CreateProject: `$.Result.ProjectId` present; `DescribeProjects` confirms.
- [ ] DeleteProject: `DescribeProjects` returns not found within poll budget.
- [ ] CreateTopic: `DescribeTopics` shows the topic with expected partition count.
- [ ] SearchLog: query returns expected log entries.
...
- [ ] DeleteProject ALL topics + shippers + logs LOST.
- [ ] DeleteTopic — log data and index LOST.
- [ ] SearchLog query: NEVER include user credentials in query strings.
...
- [ ] Full `ve sls` command line (with resolved values, NOT templates)
...
- [ ] **Dual-path**: BOTH `ve sls` CLI step AND Go SDK fallback step exist for every operation.
...
| Version | Date | Change |
|---|---|---|
| 1.0.0 | 2026-06-04 | Initial GCL rubric for ve-sls-ops (5 dimensions, 4-tier operation classification, product-specific safety + correctness + spec-compliance checks) |
