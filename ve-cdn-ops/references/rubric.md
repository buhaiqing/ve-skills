...
description: >-
  GCL rubric instance for ve-cdn-ops. Use to score Generator outputs
  ...
  Destructive: DeleteDomain; State-changing: StartDomain, StopDomain, CreateDomain, ModifyDomainConfig, SetOrigin, SubmitRefreshTask, SubmitDirRefreshTask; Mutating: PreLoadCache; Read-only: DescribeDomains, DescribeDomainDetail, DescribeCdnData, DescribeCdnOrigin, DescribeCdnTopData, DescribeContentQuota
license: MIT
metadata:
  author: volcengine
  version: "1.0.0"
...
| Tier | Operations in `ve-cdn-ops` | `max_iter` | Safety floor |
|---|---|---|---|
| **Destructive** | `DeleteDomain` | 5 | 1.0 (mandatory) |
| **State-changing** | `StartDomain, StopDomain, CreateDomain, ModifyDomainConfig, SetOrigin, SubmitRefreshTask, SubmitDirRefreshTask` | 5 | 1.0 (mandatory) |
...
- [ ] CreateDomain: `$.Result.DomainId` present; `DescribeDomains` confirms domain with expected config.
- [ ] DeleteDomain: `DescribeDomains` returns not found within poll budget.
- [ ] ModifyDomainConfig: modified config reflects in `DescribeDomainDetail`.
...
- [ ] DeleteDomain — CDN domain config + DNS mapping LOST.
- [ ] StopDomain — traffic stops serving.
- [ ] SubmitRefreshTask — purge irreversible until TTL expires.
...
- [ ] Full `ve cdn` command line (with resolved values, NOT templates)
...
- [ ] **Dual-path**: BOTH `ve cdn` CLI step AND Go SDK fallback step exist for every operation.
...
| Version | Date | Change |
|---|---|---|
| 1.0.0 | 2026-06-04 | Initial GCL rubric for ve-cdn-ops (5 dimensions, 4-tier operation classification, product-specific safety + correctness + spec-compliance checks) |
