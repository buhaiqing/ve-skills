...
description: >-
  GCL rubric instance for ve-vke-ops. Use to score Generator outputs
...
- [ ] CreateCluster: `$.Result.ClusterId` present; status `Running` within poll budget.
- [ ] DeleteCluster: `DescribeCluster` returns not found within poll budget.
- [ ] CreateNodePool: `ListNodePools` shows the new pool with expected config.
...
- [ ] DeleteCluster warn ALL k8s workloads + data + config LOST.
- [ ] DeleteNodePool warn pods on those nodes rescheduled.
...
- [ ] Full `ve vke` command line (with resolved values, NOT templates)
...
- [ ] **Dual-path**: BOTH `ve vke` CLI step AND Go SDK fallback step exist for every operation.
...
| Version | Date | Change |
|---|---|---|
| 1.0.0 | 2026-06-04 | Initial GCL rubric for ve-vke-ops (5 dimensions, 4-tier operation classification, product-specific safety + correctness + spec-compliance checks) |
