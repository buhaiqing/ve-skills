...
| **Dedup** | Before adding, check if pattern exists (match by `(skill, pattern)`). If exists, increment `count`. Aligns with `vet gcl trace` (orig. `scripts/gcl_trace_aggregate.py`, now deprecated) `update_failure_patterns_file` and the `incident-loop-agent` rubric. |
