...
description: >-
  GCL rubric instance for ve-mongodb-ops. Use to score Generator outputs
  ...
  Destructive: DeleteDBInstance; State-changing: ModifyDBInstanceSpec, RestartDBInstance, ModifyDBInstanceIPList, CreateDatabase, CreateDBAccount; Mutating: CreateDBInstance, CreateBackup, RestoreToNewInstance; Read-only: DescribeDBInstances, DescribeDBInstanceDetail, DescribeDBAccounts, DescribeDatabases, DescribeBackups
license: MIT
metadata:
  author: volcengine
  version: "1.0.0"
...
| Tier | Operations in `ve-mongodb-ops` | `max_iter` | Safety floor |
|---|---|---|---|
...
- [ ] Full `ve mongodb` command line (with resolved values, NOT templates)
...
- [ ] **Dual-path**: BOTH `ve mongodb` CLI step AND Go SDK fallback step exist for every operation.
...
| Version | Date | Change |
|---|---|---|
| 1.0.0 | 2026-06-04 | Initial GCL rubric for ve-mongodb-ops (5 dimensions, 4-tier operation classification, product-specific safety + correctness + spec-compliance checks) |
