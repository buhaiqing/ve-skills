...
description: >-
  GCL rubric instance for ve-elasticsearch-ops. Use to score Generator outputs
  ...
  Destructive: DeleteInstance, DeleteIndex (with caution for cluster-wide index), DeleteSnapshot; State-changing: ModifyInstanceSpec, RestartInstance, UpgradeVersion, EnableKibana, DisableKibana, InstallPlugin, UninstallPlugin, ModifyInstanceChargeType; Mutating: CreateInstance, CreateIndex, CreateSnapshot, KibanaCreateUser; Read-only: DescribeInstances, DescribeIndices, DescribeSnapshots, DescribePlugins, DescribeKibana
license: MIT
metadata:
  author: volcengine
  version: "1.0.0"
...
| Tier | Operations in `ve-elasticsearch-ops` | `max_iter` | Safety floor |
|---|---|---|---|
...
- [ ] Full `ve elasticsearch` command line (with resolved values, NOT templates)
...
- [ ] **Dual-path**: BOTH `ve elasticsearch` CLI step AND Go SDK fallback step exist for every operation.
...
| Version | Date | Change |
|---|---|---|
| 1.0.0 | 2026-06-04 | Initial GCL rubric for ve-elasticsearch-ops (5 dimensions, 4-tier operation classification, product-specific safety + correctness + spec-compliance checks) |
