# SecurityOps — FunctionGraph Security Operations

> SecurityOps content for non-security-critical skills per TE-7.
> **Purpose**: Function code access control, trigger authentication, secret environment variable management, and execution role hardening for FunctionGraph resources.

## Security Baseline Checklist

```markdown
## FunctionGraph Security Baseline — Current

### Access Control
- [ ] IAM policies scoped to fg required APIs only
- [ ] No hardcoded credentials in scripts/configs (use {{env.*}} placeholders)
- [ ] API key rotation schedule documented and tracked
- [ ] Service accounts mapped to specific FunctionGraph operations

### Network Security
- [ ] FunctionGraph endpoint access restricted to trusted networks
- [ ] Network ACLs / security groups scoped to FunctionGraph data plane
- [ ] Access logging enabled for audit trail

### Data Protection
- [ ] Data at rest encryption verified
- [ ] Data in transit encryption enforced (TLS 1.2+)
- [ ] Data retention policy aligned with compliance requirements

### Audit & Monitoring
- [ ] FunctionGraph operation logging enabled
- [ ] Security-related events have separate alarm rules
- [ ] Unauthorized access attempts trigger alerts via ve-cms-ops
```
