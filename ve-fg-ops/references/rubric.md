...
description: >-
  GCL rubric instance for ve-fg-ops. Use to score Generator outputs
  ...
  Destructive: DeleteFunction, DeleteTrigger; State-changing: UpdateFunction, PublishVersion, CreateTrigger; Mutating: CreateFunction; Read-only: GetFunction, ListFunctions, InvokeFunction
license: MIT
metadata:
  author: volcengine
  version: "1.0.0"
...
| Tier | Operations in `ve-fg-ops` | `max_iter` | Safety floor |
|---|---|---|---|
| **Destructive** | `DeleteFunction, DeleteTrigger` | 3 | 1.0 (mandatory) |
| **State-changing** | `UpdateFunction, PublishVersion, CreateTrigger` | 3 | 1.0 (mandatory) |
...
- [ ] CreateFunction: `$.Result.FunctionId` present; `GetFunction` confirms status `Active`.
- [ ] DeleteFunction: `GetFunction` returns not found within poll budget.
- [ ] UpdateFunction: updated config reflects in `GetFunction`.
...
- [ ] DeleteFunction warn ALL versions + aliases + triggers LOST.
- [ ] DeleteTrigger warn function stops responding to events.
- [ ] UpdateFunction: warn about in-flight invocations.
...
- [ ] Full `ve fg` command line (with resolved values, NOT templates)
...
- [ ] **Dual-path**: BOTH `ve fg` CLI step AND Go SDK fallback step exist for every operation.
...
| Version | Date | Change |
|---|---|---|
| 1.0.0 | 2026-06-04 | Initial GCL rubric for ve-fg-ops (5 dimensions, 4-tier operation classification, product-specific safety + correctness + spec-compliance checks) |
