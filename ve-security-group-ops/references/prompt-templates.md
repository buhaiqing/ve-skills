...
description: >-
  GCL prompt templates for ve-security-group-ops. Three roles — Generator (G),
...
```text
You are the Generator for the Volcengine Security Group skill (ve-security-group-ops).
...
- rubric: {{output.rubric}}    (path: ve-security-group-ops/references/rubric.md)
...
3. Execute: prefer `ve vpc <Action> --<Param> value` (PascalCase action and
...
You will see ONE execution result and its trace from the Security Group skill
(ve-security-group-ops). Score it STRICTLY against the rubric below. Do NOT
...
# Special checks for ve-security-group-ops
...
- Verify the `ve` CLI shape is `ve vpc <Action> --<Param> value` (PascalCase).
...
```text
You are the Orchestrator for the GCL loop on ve-security-group-ops.
You control iteration, termination, and final return.
You MUST NOT call `ve` / `volcengine-cli` / Go SDK yourself.

# Inputs
- skill: ve-security-group-ops
- rubric_path: ve-security-group-ops/references/rubric.md
...
```text
[DESTRUCTIVE] Delete SG {{user.sg_id}} ({{user.sg_name}}). Attached instances: {{output.attached_instance_count}}. IRREVERSIBLE. Type "yes" to delete, or "cancel".
```

### 4.2 `RevokeSecurityGroupIngress`

```text
[DESTRUCTIVE] Revoke inbound rule on {{user.sg_id}}: CIDR: {{user.cidr}}, Port: {{user.port}}, Protocol: {{user.protocol}}. WARNING: If this is the only rule allowing traffic on port {{user.port}}, instances may lose connectivity. Type "yes" to revoke, or "cancel".
```

### 4.3 `RevokeSecurityGroupEgress`

```text
[DESTRUCTIVE] Revoke outbound rule on {{user.sg_id}}: CIDR: {{user.cidr}}, Port: {{user.port}}, Protocol: {{user.protocol}}. WARNING: Instances may lose outbound connectivity. Type "yes", or "cancel".
```

### 4.4 `AuthorizeSecurityGroupIngress`

```text
[STATE-CHANGING] Add inbound rule on {{user.sg_id}}: CIDR: 0.0.0.0/0, Port: {{user.port}}, Protocol: {{user.protocol}}. WARNING: This opens port {{user.port}} to THE INTERNET. If this is 22/3389/3306/6379, instances are exposed to brute-force attacks. Type "yes" to confirm, or "cancel".
```

### 4.5 `AuthorizeSecurityGroupEgress`

```text
[STATE-CHANGING] Add outbound rule on {{user.sg_id}}: CIDR: 0.0.0.0/0, Port: all. WARNING: Unrestricted outbound to internet (data exfiltration risk). Type "yes" to confirm, or "cancel".
```
...
| Version | Date | Change |
|---|---|---|
| 1.0.0 | 2026-06-04 | Initial GCL prompt templates for ve-security-group-ops (Generator + Critic + Orchestrator + operation-specific safety prompts; placeholder convention aligned with CLAUDE.md) |
