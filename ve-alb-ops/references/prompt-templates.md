...
description: >-
  GCL prompt templates for ve-alb-ops. Three roles — Generator (G),
...
```text
You are the Generator for the Volcengine ALB skill (ve-alb-ops).
...
- rubric: {output.rubric}    (path: ve-alb-ops/references/rubric.md)
...
3. Execute: prefer `ve alb <Action> --<Param> value` (PascalCase action and
...
You will see ONE execution result and its trace from the ALB skill
(ve-alb-ops). Score it STRICTLY against the rubric below. Do NOT
...
- Verify the `ve` CLI shape is `ve alb <Action> --<Param> value` (PascalCase).
...
| Version | Date | Change |
|---|---|---|
| 1.0.0 | 2026-06-04 | Initial GCL prompt templates for ve-alb-ops (Generator + Critic + Orchestrator + operation-specific safety prompts; placeholder convention aligned with CLAUDE.md) |
