...
description: >-
  GCL rubric instance for ve-alb-ops. Use to score Generator outputs
  ...
  Destructive: DeleteLoadBalancer, DeleteListener, DeleteRule, DeleteServerGroup; State-changing: AddBackendServers, RemoveBackendServers, ModifyListenerAttributes, ModifyRuleAttributes; Mutating: CreateLoadBalancer, CreateListener, CreateRule, CreateServerGroup; Read-only: DescribeLoadBalancers, DescribeListeners, DescribeRules, DescribeServerGroups
license: MIT
metadata:
  author: volcengine
  version: "1.0.0"
...
- [ ] ModifyListenerAttributes: modified field reflects in `DescribeListeners` after operation completes.
...
- [ ] DeleteLoadBalancer warn ALL listeners/rules/server-groups lost — traffic cut.
- [ ] DeleteServerGroup in use by listener: warn about routing disruption.
...
- [ ] Full `ve alb` command line (with resolved values, NOT templates)
...
- [ ] **Dual-path**: BOTH `ve alb` CLI step AND Go SDK fallback step exist for every operation.
...
| Version | Date | Change |
|---|---|---|
| 1.0.0 | 2026-06-04 | Initial GCL rubric for ve-alb-ops (5 dimensions, 4-tier operation classification, product-specific safety + correctness + spec-compliance checks) |
