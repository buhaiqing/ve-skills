...
description: >-
  GCL rubric instance for ve-rds-pg-ops. Use to score Generator outputs
  ...
  Destructive: DeleteDBInstance, DeleteDBAccount; State-changing: ModifyDBInstanceSpec, ModifyDBInstanceParameter, ModifyDBInstanceIPList, RestartDBInstance; Mutating: CreateDBInstance, CreateDBAccount, CreateBackup, RestoreToNewInstance, CreateReadOnlyNode; Read-only: DescribeDBInstances, DescribeDBInstanceDetail, DescribeDBInstanceParameters, DescribeAccounts, DescribeBackups
license: MIT
metadata:
  author: volcengine
  version: "1.0.0"
...
| Tier | Operations in `ve-rds-pg-ops` | `max_iter` | Safety floor |
|---|---|---|---|
...
- [ ] DeleteDBAccount: account no longer listed.
...
- [ ] Full `ve rds_postgresql` command line (with resolved values, NOT templates)
...
- [ ] **Dual-path**: BOTH `ve rds_postgresql` CLI step AND Go SDK fallback step exist for every operation.
...
| Version | Date | Change |
|---|---|---|
| 1.0.0 | 2026-06-04 | Initial GCL rubric for ve-rds-pg-ops (5 dimensions, 4-tier operation classification, product-specific safety + correctness + spec-compliance checks) |
