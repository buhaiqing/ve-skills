...
description: >-
  GCL rubric instance for ve-dns-ops. Use to score Generator outputs
  ...
  Destructive: DeleteDNSRecord, DeleteDNSDomain; State-changing: CreateDNSRecord, ModifyDNSRecord, ModifyDNSDomain; Mutating: CreateDNSDomain; Read-only: DescribeDNSDomains, DescribeDNSRecords, DescribeDomainStatistics
license: MIT
metadata:
  author: volcengine
  version: "1.0.0"
...
| Tier | Operations in `ve-dns-ops` | `max_iter` | Safety floor |
|---|---|---|---|
| **Destructive** | `DeleteDNSRecord, DeleteDNSDomain` | 5 | 1.0 (mandatory) |
| **State-changing** | `CreateDNSRecord, ModifyDNSRecord, ModifyDNSDomain` | 5 | 1.0 (mandatory) |
...
- [ ] CreateDNSDomain: `$.Result.DomainId` present; `DescribeDNSDomains` confirms the domain.
- [ ] DeleteDNSDomain: `DescribeDNSDomains` returns not found within poll budget.
- [ ] CreateDNSRecord: `$.Result.RecordId` present; `DescribeDNSRecords` shows the new record.
...
- [ ] DeleteDNSDomain ALL records under domain LOST — DNS resolution breaks.
- [ ] DeleteDNSRecord — specific record stops resolving.
- [ ] ModifyDNSRecord with wrong value causes resolution failures.
...
- [ ] Full `ve dns` command line (with resolved values, NOT templates)
...
- [ ] **Dual-path**: BOTH `ve dns` CLI step AND Go SDK fallback step exist for every operation.
...
| Version | Date | Change |
|---|---|---|
| 1.0.0 | 2026-06-04 | Initial GCL rubric for ve-dns-ops (5 dimensions, 4-tier operation classification, product-specific safety + correctness + spec-compliance checks) |
