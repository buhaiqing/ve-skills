...
description: >-
  GCL prompt templates for ve-nas-ops. Three roles — Generator (G),
...
```text
You are the Generator for the Volcengine NAS skill (ve-nas-ops).
...
- rubric: {output.rubric}    (path: ve-nas-ops/references/rubric.md)
...
3. Execute: prefer `ve nas <Action> --<Param> value` (PascalCase action and
...
You will see ONE execution result and its trace from the NAS skill
(ve-nas-ops). Score it STRICTLY against the rubric below. Do NOT
...
- Verify the `ve` CLI shape is `ve nas <Action> --<Param> value` (PascalCase).
...
| Version | Date | Change |
|---|---|---|
| 1.0.0 | 2026-06-04 | Initial GCL prompt templates for ve-nas-ops (Generator + Critic + Orchestrator + operation-specific safety prompts; placeholder convention aligned with CLAUDE.md) |
