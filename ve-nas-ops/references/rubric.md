...
description: >-
  GCL rubric instance for ve-nas-ops. Use to score Generator outputs
  ...
  Destructive: DeleteFileSystem, DeleteMountTarget; State-changing: ModifyFileSystem, ModifyMountTarget; Mutating: CreateFileSystem, CreateMountTarget; Read-only: DescribeFileSystems, ListMountTargets
license: MIT
metadata:
  author: volcengine
  version: "1.0.0"
...
| Tier | Operations in `ve-nas-ops` | `max_iter` | Safety floor |
|---|---|---|---|
| **Destructive** | `DeleteFileSystem, DeleteMountTarget` | 3 | 1.0 (mandatory) |
| **State-changing** | `ModifyFileSystem, ModifyMountTarget` | 3 | 1.0 (mandatory) |
...
- [ ] CreateFileSystem: `$.Result.FileSystemId` present; `DescribeFileSystems` confirms status `Running`.
- [ ] DeleteFileSystem: `DescribeFileSystems` returns not found within poll budget.
- [ ] CreateMountTarget: `$.Result.MountTargetId` present; `ListMountTargets` confirms.
...
- [ ] DeleteFileSystem warn ALL data LOST (files, snapshots, mount targets).
- [ ] DeleteMountTarget warn instance disconnection.
...
- [ ] Full `ve nas` command line (with resolved values, NOT templates)
...
- [ ] **Dual-path**: BOTH `ve nas` CLI step AND Go SDK fallback step exist for every operation.
...
| Version | Date | Change |
|---|---|---|
| 1.0.0 | 2026-06-04 | Initial GCL rubric for ve-nas-ops (5 dimensions, 4-tier operation classification, product-specific safety + correctness + spec-compliance checks) |
