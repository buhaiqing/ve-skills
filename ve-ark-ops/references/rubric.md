...
description: >-
  GCL rubric instance for ve-ark-ops. Use to score Generator outputs
  ...
  Destructive: DeleteEndpoint, DeleteDataset; State-changing: CreateEndpoint, StopEndpoint, CreateTrainingJob, StopTrainingJob, CreateEvaluationJob; Read-only: ListEndpoints, DescribeEndpoint, ListModels, ListTrainingJobs, ListDatasets
license: MIT
metadata:
  author: volcengine
  version: "1.0.0"
...
| Tier | Operations in `ve-ark-ops` | `max_iter` | Safety floor |
|---|---|---|---|
| **Destructive** | `DeleteEndpoint, DeleteDataset` | 3 | 1.0 (mandatory) |
| **State-changing** | `CreateEndpoint, StopEndpoint, CreateTrainingJob, StopTrainingJob, CreateEvaluationJob` | 3 | 1.0 (mandatory) |
...
- [ ] CreateEndpoint: `$.Result.EndpointId` present; status `Running` within poll budget.
- [ ] DeleteEndpoint: `DescribeEndpoint` returns not found within poll budget.
- [ ] StopEndpoint: final status `Stopped`.
...
- [ ] DeleteEndpoint warn model inference endpoints stop — production apps affected.
- [ ] StopTrainingJob warn training loses progress (checkpoint?).
- [ ] CreateEndpoint expensive model: warn about ongoing cost.
...
- [ ] Full `ve ark` command line (with resolved values, NOT templates)
...
- [ ] **Dual-path**: BOTH `ve ark` CLI step AND Go SDK fallback step exist for every operation.
...
| Version | Date | Change |
|---|---|---|
| 1.0.0 | 2026-06-04 | Initial GCL rubric for ve-ark-ops (5 dimensions, 4-tier operation classification, product-specific safety + correctness + spec-compliance checks) |
