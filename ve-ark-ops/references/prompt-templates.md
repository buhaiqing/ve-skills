...
description: >-
  GCL prompt templates for ve-ark-ops. Three roles — Generator (G),
...
```text
You are the Generator for the Volcengine Ark skill (ve-ark-ops).
...
- rubric: {{output.rubric}}    (path: ve-ark-ops/references/rubric.md)
...
3. Execute: prefer `ve ark <Action> --<Param> value` (PascalCase action and
...
You will see ONE execution result and its trace from the Ark skill
(ve-ark-ops). Score it STRICTLY against the rubric below. Do NOT
...
- Verify the `ve` CLI shape is `ve ark <Action> --<Param> value` (PascalCase).
...
| Version | Date | Change |
|---|---|---|
| 1.0.0 | 2026-06-04 | Initial GCL prompt templates for ve-ark-ops (Generator + Critic + Orchestrator + operation-specific safety prompts; placeholder convention aligned with CLAUDE.md) |
