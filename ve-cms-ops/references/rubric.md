...
description: >-
  GCL rubric instance for ve-cms-ops. Use to score Generator outputs
  ...
  Destructive: DeleteAlarmRule, DeleteAlarmStrategy; State-changing: CreateAlarmRule, ModifyAlarmRule, DisableAlarmRule, CreateAlarmStrategy, ModifyAlarmStrategy; Mutating: — (all alarm operations are state-changing); Read-only: DescribeAlarmRules, DescribeAlarmStrategies, GetMetricData
license: MIT
metadata:
  author: volcengine
  version: "1.0.0"
...
| Tier | Operations in `ve-cms-ops` | `max_iter` | Safety floor |
|---|---|---|---|
| **Destructive** | `DeleteAlarmRule, DeleteAlarmStrategy` | 3 | 1.0 (mandatory) |
| **State-changing** | `CreateAlarmRule, ModifyAlarmRule, DisableAlarmRule, CreateAlarmStrategy, ModifyAlarmStrategy` | 3 | 1.0 (mandatory) |
...
- [ ] CreateAlarmRule: `$.Result.RuleId` present; `DescribeAlarmRules` confirms the rule with expected metric/conditions.
- [ ] DeleteAlarmRule: `DescribeAlarmRules` returns not found within poll budget.
- [ ] ModifyAlarmRule: modified field reflects in `DescribeAlarmRules`.
...
- [ ] DeleteAlarmRule warn monitoring blackout for affected resources.
- [ ] DisableAlarmRule warn no alert notification.
- [ ] CreateAlarmRule with empty notification channel: warn no notification.
...
- [ ] Full `ve cms` command line (with resolved values, NOT templates)
...
- [ ] **Dual-path**: BOTH `ve cms` CLI step AND Go SDK fallback step exist for every operation.
...
| Version | Date | Change |
|---|---|---|
| 1.0.0 | 2026-06-04 | Initial GCL rubric for ve-cms-ops (5 dimensions, 4-tier operation classification, product-specific safety + correctness + spec-compliance checks) |
