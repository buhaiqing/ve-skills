...
description: >-
  GCL rubric instance for ve-nat-ops. Use to score Generator outputs
  ...
  Destructive: DeleteNatGateway, DeleteSnatRule, DeleteDnatRule; State-changing: CreateSnatRule, CreateDnatRule; Mutating: CreateNatGateway, ModifyNatGatewayAttribute; Read-only: DescribeNatGateways, DescribeSnatRules, DescribeDnatRules
license: MIT
metadata:
  author: volcengine
  version: "1.0.0"
...
| Tier | Operations in `ve-nat-ops` | `max_iter` | Safety floor |
|---|---|---|---|
| **Destructive** | `DeleteNatGateway, DeleteSnatRule, DeleteDnatRule` | 3 | 1.0 (mandatory) |
| **State-changing** | `CreateSnatRule, CreateDnatRule` | 3 | 1.0 (mandatory) |
...
- [ ] CreateNatGateway: `$.Result.NatGatewayId` present; status `Available` within poll budget.
- [ ] DeleteNatGateway: `DescribeNatGateways` returns not found within poll budget.
- [ ] CreateSnatRule: `DescribeSnatRules` shows the new rule.
...
- [ ] DeleteNatGateway warn about ALL SNAT/DNAT rules removed — instances lose internet access.
- [ ] CreateDnatRule with 0.0.0.0/0 on sensitive port: warn.
...
- [ ] Full `ve nat` command line (with resolved values, NOT templates)
...
- [ ] **Dual-path**: BOTH `ve nat` CLI step AND Go SDK fallback step exist for every operation.
...
| Version | Date | Change |
|---|---|---|
| 1.0.0 | 2026-06-04 | Initial GCL rubric for ve-nat-ops (5 dimensions, 4-tier operation classification, product-specific safety + correctness + spec-compliance checks) |
