...
description: >-
  GCL rubric instance for ve-clb-ops. Use to score Generator outputs
  ...
  Destructive: DeleteLoadBalancer, DeleteListener; State-changing: RemoveBackendServers, AddBackendServers, SetHealthCheckConfig, ModifyLoadBalancerAttributes; Mutating: CreateLoadBalancer, CreateListener; Read-only: DescribeLoadBalancers, DescribeListeners, DescribeBackendServers
license: MIT
metadata:
  author: volcengine
  version: "1.0.0"
...
| Tier | Operations in `ve-clb-ops` | `max_iter` | Safety floor |
|---|---|---|---|
| **Destructive** | `DeleteLoadBalancer, DeleteListener` | 3 | 1.0 (mandatory) |
| **State-changing** | `RemoveBackendServers, AddBackendServers, SetHealthCheckConfig, ModifyLoadBalancerAttributes` | 3 | 1.0 (mandatory) |
...
- [ ] CreateLoadBalancer: `$.Result.LoadBalancerId` present; status `Active` within poll budget.
- [ ] DeleteLoadBalancer: `DescribeLoadBalancers` returns not found within poll budget.
- [ ] ModifyLoadBalancerAttributes: modified field reflects in `DescribeLoadBalancers`.
...
- [ ] DeleteLoadBalancer warn traffic to ALL backends cut.
- [ ] RemoveBackendServers >50%: warn about capacity drop.
...
- [ ] Full `ve clb` command line (with resolved values, NOT templates)
...
- [ ] **Dual-path**: BOTH `ve clb` CLI step AND Go SDK fallback step exist for every operation.
...
| Version | Date | Change |
|---|---|---|
| 1.0.0 | 2026-06-04 | Initial GCL rubric for ve-clb-ops (5 dimensions, 4-tier operation classification, product-specific safety + correctness + spec-compliance checks) |
