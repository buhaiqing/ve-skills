...
func TestTranspile_AboveThreshold(t *testing.T) {
	...
	g, ok := Transpile(p)
	if !ok {
		t.Fatal("expected true for count >= 10")
	}
	...
	if g.SourceCount != 15 {
		t.Errorf("SourceCount = %d, want %d", g.SourceCount, 15)
	}
	if g.ID == "" {
...
func TestTranspile_HardLevel(t *testing.T) {
	...
	g, ok := Transpile(p)
	if !ok {
		t.Fatal("expected true for count >= 30")
	}
	if g.Severity != "high" {
...
|----------|-----------------|------------|-----|-------|
| Alarm triage | routing_mismatch | wrong skill | verify graph | 15 |
| Diagnosis | evidence_overfetch | >20 calls | cap at 15 | 30 |
| Reflexion | pattern_undercount | forgotten | scan every run | 10 |
| Alarm triage | correlated_alarm_missed | only loudest | run correlation | 5 |
`
	if err := os.WriteFile(inPath, []byte(content), 0o644); err != nil {
		t.Fatal(err)
	}
	...
	if n != 3 {
		t.Errorf("expected 3 guardrails (counts 15,30,10), got %d", n)
	}
	...
	if !strings.Contains(outStr, "auto-REFUSE") {
		t.Error("output should contain 'auto-REFUSE' for count=30")
	}
