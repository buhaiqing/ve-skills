...
description: >-
  GCL prompt templates for ve-skill-generator. Three roles — Generator (G),
...
```text
You are the Generator for the Volcengine Skill Generator skill (ve-skill-generator).
...
- rubric: {{output.rubric}}    (path: ve-skill-generator/references/rubric.md)
...
3. Execute: prefer `ve skill-generator <Action> --<Param> value` (PascalCase action and
...
You will see ONE execution result and its trace from the Skill Generator skill
(ve-skill-generator). Score it STRICTLY against the rubric below. Do NOT
...
# Special checks for ve-skill-generator
...
- Verify the `ve` CLI shape is `ve skill-generator <Action> --<Param> value` (PascalCase).
...
```text
You are the Orchestrator for the GCL loop on ve-skill-generator.
...
- rubric_path: ve-skill-generator/references/rubric.md
...
No destructive or state-changing operations currently defined for this skill.
...
| Version | Date | Change |
|---|---|---|
| 1.0.0 | 2026-06-04 | Initial GCL prompt templates for ve-skill-generator (Generator + Critic + Orchestrator + operation-specific safety prompts; placeholder convention aligned with CLAUDE.md) |
