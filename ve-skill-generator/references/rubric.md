...
description: >-
  GCL rubric instance for ve-skill-generator. Use to score Generator outputs
  ...
  State-changing: GenerateSkill, UpdateSkill; Read-only: AnalyzeOpenAPI, ReviewSkill
license: MIT
metadata:
  author: volcengine
  version: "1.0.0"
...
| Tier | Operations in `ve-skill-generator` | `max_iter` | Safety floor |
|---|---|---|---|
| **Destructive** | `— (generation is additive)` | 3 | 1.0 (mandatory) |
...
- [ ] GenerateSkill: output SKILL.md has ## Quality Gate (GCL) section with 4-tier classification.
- [ ] GenerateSkill: references/rubric.md has 5 dimensions with product-specific rules.
- [ ] GenerateSkill: references/prompt-templates.md has G/C/O prompts + safety prompts.
- [ ] GenerateSkill: No real credentials in examples — only {{env.*}} and <masked>.
...
- [ ] Generated skill MUST NOT contain real credentials in examples — only <masked> and {{env.*}} placeholders.
- [ ] Generated rubric MUST include 5 dimensions.
- [ ] Generated prompt-templates MUST include Critic that hides user request.
- [ ] Generated skill MUST reference references/rubric.md and references/prompt-templates.md.
- [ ] VOLCENGINE_SECRET_KEY never in generated output.
...
- [ ] Full `ve skill-generator` command line (with resolved values, NOT templates)
...
- [ ] **Dual-path**: BOTH `ve skill-generator` CLI step AND Go SDK fallback step exist for every operation.
...
| Version | Date | Change |
|---|---|---|
| 1.0.0 | 2026-06-04 | Initial GCL rubric for ve-skill-generator (5 dimensions, 4-tier operation classification, product-specific safety + correctness + spec-compliance checks) |
